...
import { type Register, type Moved, type OnMove, sizeLeast, tileSizeKey, tileEdge, groupHead } from './layout';

export const Tile = memo(function Tile({ places, nth, born, carried, lit, stood, register, moved, onMove, zoom, depth = 0, within }: { places: Place[]; nth: number; born: number | null; carried: number | null; lit: number[]; stood: number | null; register: Register; moved: Moved; onMove: OnMove; zoom: number; depth?: number; within?: Region }) {
  ...
  const shift = moved[name] ?? { x: 0, y: 0 };
  ...
  const onDragged = (event: React.PointerEvent<HTMLElement>): void => {
    ...
    if (!holder) { onMove(name, wantX, wantY); return; }
    ...
    onMove(name, within(wantX, restX, tile.offsetWidth, holder.clientWidth, tileEdge), within(wantY, restY, tile.offsetHeight, holder.clientHeight, depth === 0 ? groupHead : tileEdge));
  };
  ...
  const onSized = (event: React.PointerEvent<HTMLElement>): void => {
    ...
    if (!from) return;
    event.stopPropagation();
    onMove(tileSizeKey(name), Math.max(from.fromX + (event.clientX - from.x) / zoom, sizeLeast), Math.max(from.fromY + (event.clientY - from.y) / zoom, sizeLeast));
  };
  const onSizeEnd = (event: React.PointerEvent<HTMLElement>): void => { event.stopPropagation(); sizing.current = null; };
  const size = moved[tileSizeKey(name)];
  ...
  return (
    <Box className="tile" ref={(element: HTMLElement | null) => register(nth, element)} onPointerDown={onDown} onPointerMove={onDragged} onPointerUp={onUp} onPointerCancel={onUp} sx={{
      ...
      {inside.length > 0 && (
        <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap', justifyContent: 'center', maxWidth: size ? 'none' : 280 }}>
          {inside.map(({ at }) => <Tile key={at} places={places} nth={at} born={born} carried={carried} lit={lit} stood={stood} register={register} moved={moved} onMove={onMove} zoom={zoom} depth={depth + 1} within={within ?? places[nth].region} />)}
        </Box>
      )}
    </Box>
  );
