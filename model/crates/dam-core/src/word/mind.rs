...
pub fn verb_present(mind: &CursorMind, word: &str) -> Option<String> {
    told_past(&mind.tree, word).map(|n| mind.tree.node(n).name.to_string())
}
because!(verb_present, WordReading, "the verb a past word is the past of, as the seeds state it, flew of fly, so what a deed is written under is the one word for every telling of it");
